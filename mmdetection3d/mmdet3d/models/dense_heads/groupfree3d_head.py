...
from mmdet3d.models.model_utils import DisARM
...
class GroupFree3DHead(BaseModule):
    ...
    def forward(self, feat_dict, sample_mod):
        ...
        if self.disarm_module:
            relation_features = self.disarm_module(candidate_xyz, candidate_features)

            results['relation'] = relation_features
            # add relation_features and features
            candidate_features = relation_features['relation'] + candidate_features
    ...
    def loss(self,
        ...
        if self.relation_anchor_loss:
            relation_anchor_loss = self.relation_anchor_loss(
                bbox_preds['relation']['rn_anchor_objectness'],
                objectness_targets,  # objectness_targets
                weight=objectness_weights, # objectness_weights
            )
